...
<form id="kbSearch" action="kb.php" method="get">
    ...
    <div>
        ...
        <select name="cid" id="cid">
            <option value="">&mdash; все категории &mdash;</option>
            <?php
    ...
    <div>
        <select name="topicId" style="width:350px;" id="topic-id">
            <option value="">&mdash; все темы обращения &mdash;</option>
            <?php
...
if($_REQUEST['q'] || $_REQUEST['cid'] || $_REQUEST['topicId']) { //Search.
    ...
    echo "<div><strong>Результаты поиска</strong></div><div class='clear'></div>";
    if(($res=db_query($sql)) && db_num_rows($res)) {
        echo '<div id="faq">
                <ol>';
        while($row=db_fetch_array($res)) {
        ...
        echo '<strong class="faded">Ничего не найдено.</strong>';
    }
    ...
    $sql='SELECT cat.category_id, cat.name, cat.description, cat.ispublic, count(faq.faq_id) as faqs '
        ...
        .' ORDER BY cat.name';
    if(($res=db_query($sql)) && db_num_rows($res)) {
        echo '<div>Нажмите на категорию для просмотра вопросов и ответов.</div>
                <ul id="kb">';
        while($row=db_fetch_array($res)) {
            ...
            echo sprintf('
                ...
                ($row['ispublic']?'Опубликовано':'Не опубликовано'),
                Format::safe_html($row['description']));
        }
        echo '</ul>';
    } else {
        echo 'В данный момент ничего нет.';
    }
}
?>
</div>
