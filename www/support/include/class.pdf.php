...
class Ticket2PDF extends FPDF
{
	...
	function Header() {
        ...
        $this->Cell(0, 5, 'Сформировано '.Format::date($cfg->getDateTimeFormat(), Misc::gmtime(), $_SESSION['TZ_OFFSET'], $_SESSION['TZ_DST']), 0, 0, 'L');
        $this->Cell(0, 5, 'Дата и время по часовому поясу GMT '.$_SESSION['TZ_OFFSET'], 0, 1, 'R');
		$this->Ln(10);
	}
	...
	function Footer() {
		...
		$this->Cell(0, 7, 'Заявка #'.$this->getTicket()->getNumber().' распечатана '.$thisstaff->getUserName().' - '.date('r'), 0, 0, 'L');
		//$this->Cell(0,10,'Page '.($this->PageNo()-$this->pageOffset).' of {nb} '.$this->pageOffset.' '.$this->PageNo(),0,0,'R');
		$this->Cell(0, 7, 'Страница ' . ($this->PageNo() - $this->pageOffset), 0, 0, 'R');
	}
    ...
    function _print() {
        ...
        $this->Cell($w, 7,'Заявка #'.$ticket->getNumber(), 0, 0, 'L');
        $this->Ln(7);
        $this->cMargin = 3;
        $this->SetTextColor(0);
        ...
        $this->SetX($this->lMargin);
        $this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'Статус', 1, 0, 'L', true);
        $this->SetFont('');
        $this->Cell($c, 7, $ticket->getStatus(), 1, 0, 'L', true);
        $this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'Имя', 1, 0, 'L', true);
        $this->SetFont('');
        $this->Cell($c, 7, $ticket->getName(), 1, 1, 'L', true);
        $this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'Приоритет', 1, 0, 'L', true);
        $this->SetFont('');
        ...
        $this->SetFont('Arial', 'B', 11);
        if($ticket->isOpen()) {
            $this->Cell($l, 7, 'Назначено', 1, 0, 'L', true);
            $this->SetFont('');
            ...
            $this->Cell($l, 7, 'Закрыто', 1, 0, 'L', true);
            $this->SetFont('');
            $this->Cell($c, 7, $closedby, 1, 0, 'L', true);
        }

        $this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'Тема обращения', 1, 0, 'L', true);
        $this->SetFont('');
        $this->Cell($c, 7, $ticket->getHelpTopic(), 1, 1, 'L', true);
        $this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'SLA план', 1, 0, 'L', true);
        $this->SetFont('');
        $sla = $ticket->getSLA();
        $this->Cell($c, 7, $sla?$sla->getName():' -- ', 1, 0, 'L', true);
        $this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'Посл. ответ', 1, 0, 'L', true);
        $this->SetFont('');
        ...
        if($ticket->isOpen()) {
            ...
            $this->Cell($l, 7, 'Дата закрытия', 1, 0, 'L', true);
            $this->SetFont('');
            $this->Cell($c, 7, Format::db_datetime($ticket->getCloseDate()), 1, 0, 'L', true);
        }

        $this->SetFont('Arial', 'B', 11);
        $this->Cell($l, 7, 'Посл. сообщ.', 1, 0, 'L', true);
        $this->SetFont('');
        ...
        if(($entries = $ticket->getThreadEntries($types))) {
            foreach($entries as $entry) {
                ...
                if($entry['attachments']
                    ...
                    $text.="\nФайлов прикреплено: [".implode(', ',$files)."]\n";
                }
