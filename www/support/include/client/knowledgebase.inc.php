...
if($_REQUEST['q'] || $_REQUEST['cid'] || $_REQUEST['topicId']) { //Search.
    ...
    if(($res=db_query($sql)) && ($num=db_num_rows($res))) {
        echo '<div id="faq">'.$num.' записей соответсвует вашему запросу.
                <ol>';
        while($row=db_fetch_array($res)) {
            echo sprintf('
        ...
        echo '<strong class="faded">Поиск не дал результатов.</strong>';
    }
    ...
    $sql='SELECT cat.category_id, cat.name, cat.description, cat.ispublic, count(faq.faq_id) as faqs '
        ...
        .' ORDER BY cat.name';
    if(($res=db_query($sql)) && db_num_rows($res)) {
        echo '<div>Нажмите по категории, чтобы просмотреть вопросы и ответы.</div>
                <ul id="kb">';
        while($row=db_fetch_array($res)) {
