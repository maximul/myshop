...
class Ticket {
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function update($vars, &$errors) {
        ...
        $fields['topicId']  = array('type'=>'int',      'required'=>1, 'error'=>'Введите тему обращения');
        $fields['priorityId'] = array('type'=>'int',    'required'=>1, 'error'=>'Введите приоритет');
        $fields['slaId']    = array('type'=>'int',      'required'=>0, 'error'=>'Выберите SLA план');
        $fields['phone']    = array('type'=>'phone',    'required'=>0, 'error'=>'Valid phone # required');
        $fields['duedate']  = array('type'=>'date',     'required'=>0, 'error'=>'Неверная дата - должно быть в формате MM/DD/YY');

        $fields['note']     = array('type'=>'text',     'required'=>1, 'error'=>'Необходимо ввести причину изменения');

        if(!Validator::process($fields, $vars, $errors) && !$errors['err'])
            $errors['err'] = 'Заявка не изменена. Исправьте ошибки и повторите.';
        ...
        if($vars['duedate']) {
            ...
            elseif(strtotime($vars['duedate'].' '.$vars['time'])<=time())
                $errors['duedate']='Просроченная дата должна быть в будущем';
        }
